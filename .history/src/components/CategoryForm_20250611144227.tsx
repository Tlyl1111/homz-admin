...
export const CategoryForm = ({ category, onClose, onSave }: CategoryFormProps) => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    form.append("upload_preset", "YOUR_UPLOAD_PRESET"); // Thay bằng upload preset của bạn
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          {formData.iconPath && (
            <div className="mt-2">
              <img src={formData.iconPath} alt="Icon Preview" className="w-16 h-16 rounded-lg border" />
              <p className="text-xs text-gray-500 mt-1 break-all">{formData.iconPath}</p>
            </div>
          )}
        </div>
